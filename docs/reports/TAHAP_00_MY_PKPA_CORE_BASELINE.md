# Tahap 00 - MY PKPA Core Baseline

## Ringkasan

Tahap 00 mengubah baseline SI-KP legacy menjadi MY PKPA, menambahkan landing page publik, menyiapkan mode autentikasi Core HTTP, menutup manajemen akun lokal dan war secara default, serta menambahkan dokumentasi integrasi, migrasi domain, dan UX penempatan.
...
- Gap: belum ditemukan SSO/OAuth/central logout khusus MY PKPA; belum ditemukan endpoint password verification khusus `kppspa-farmasi` seperti endpoint TU.

## Perubahan MY PKPA

- Rebranding utama menjadi MY PKPA pada README, AGENTS, env examples, layout, login, role dashboard, role seeder, dan landing.
...
- `config/my_pkpa.php` menampung feature flag dan tautan akun Core.
- Middleware baru:
...
1. MY PKPA mengirim email/password ke Core `POST /api/v1/auth/login`.
2. MY PKPA membaca app access user di Core.
3. MY PKPA membuat atau memperbarui local projection minimal.
...
Landing publik mencakup hero MY PKPA, informasi PKPA, enam wahana, literatur/panduan, dan tautan login/akun Core berbasis konfigurasi.

## Status Akun Lokal

- Registrasi lokal: tidak tersedia.
- Reset password lokal: tidak tersedia sebagai public route.
- Manajemen akun lokal: dikunci default `MY_PKPA_LOCAL_ACCOUNT_MANAGEMENT_ENABLED=false`.
...
- `README.md`
- `AGENTS.md`
- `tests/Feature/Tahap00MyPkpaBaselineTest.php`
...
Catatan migration QA: dijalankan memakai SQLite sementara `C:\tmp\my_pkpa_tahap00.sqlite`, bukan database MySQL lokal.
...
- Core belum menyediakan kontrak SSO/central logout khusus MY PKPA.
