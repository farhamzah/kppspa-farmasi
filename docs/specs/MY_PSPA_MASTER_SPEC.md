...
- Nama aplikasi: MY PSPA
- Nama lengkap: MY PSPA - Sistem Informasi Program Studi Profesi Apoteker
...
MY PSPA mengelola program PKPA, kepesertaan mahasiswa, enam wahana, tempat praktik, rotasi, penempatan, jadwal, durasi, kelompok, pembimbing, logbook, kompetensi, laporan, tugas, ujian, penilaian, monitoring, rekap, dokumen PKPA, pengumuman, dan audit aktivitas bisnis.
...
Core Farmasi menjadi sumber akun, password, status akun, dan role. MY PSPA hanya menyimpan projection lokal minimal. Email tidak boleh menjadi foreign key domain.
...
Peserta baru berstatus `active` setelah lolos validasi Core: user ditemukan, akun aktif, role mahasiswa, app access MY PSPA, dan belum terdaftar di program yang sama. Status Core, NPM, nama, email, program studi, angkatan, dan status akademik disimpan sebagai snapshot.
...
Master PKPA kini menjadi sumber snapshot untuk Dokumen Internal MY PSPA. Perubahan master setelah dokumen published tidak boleh mengubah file lama. Jenis dokumen sistem disiapkan oleh `PkpaMasterSeeder`, sementara isi template, aturan nomor, dan penandatangan wajib dikonfigurasi sebelum digunakan dalam proses resmi internal.
