...
1. User mengisi email dan password di MY PKPA.
2. MY PKPA mengirim kredensial ke `POST /api/v1/auth/login`.
3. Core memverifikasi password dan status aktif.
4. MY PKPA membaca app access melalui `GET /api/v1/internal/apps/kppspa-farmasi/users/{id}/access`.
5. MY PKPA membuat/memperbarui projection lokal minimal.
...
`admin-core` tidak otomatis memberi akses MY PKPA.
...
Belum ditemukan endpoint khusus `kppspa-farmasi` untuk password verification seperti endpoint TU. Belum ditemukan central logout/SSO resmi untuk MY PKPA.
