    ...
    <section class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Unggah dan pantau status verifikasi dokumen persyaratan KP Anda pada periode pendaftaran aktif.</p>
            </div>
            @if($registration)
        ...
        @elseif($registration->canBeSubmitted())
            <section class="rounded-3xl border border-cyan-200 bg-cyan-50 p-5 shadow-sm">
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <form method="POST" action="{{ route('student.kp-registrations.submit', $registration) }}" onsubmit="return confirm('Kirim pendaftaran untuk diverifikasi?')">
                        @csrf
                ...
                <article class="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <div class="flex flex-col gap-2 lg:w-80">
                            @if($document?->file_path)
                                <a href="{{ route('student.kp-registrations.documents.download', [$registration, $document]) }}" class="rounded-2xl border border-slate-200 px-4 py-2 text-center text-sm font-bold text-slate-700 hover:bg-slate-50">Unduh</a>
                            @endif
                            @if($canUpload)
