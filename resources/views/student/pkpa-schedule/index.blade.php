...
<div class="space-y-5">
    ...
    <section class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        ...
        <p class="mt-1 text-sm text-slate-500">Jadwal ini berasal dari salinan resmi yang sudah dipublikasikan. Tanda membaca bukan persetujuan atau pengajuan perubahan.</p>
        ...
        <section class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
            ...
            <div class="mt-3 space-y-2">
                @foreach($history as $item)
                    <div class="rounded-xl border border-slate-200 px-4 py-3 text-sm">{{ $item->code }} / {{ $item->status }} / {{ optional($item->published_at)->format('d M Y H:i') ?: '-' }}</div>
                @endforeach
            </div>
        </section>
    @endif
