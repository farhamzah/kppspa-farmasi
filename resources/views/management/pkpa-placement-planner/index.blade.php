                ...
                <aside class="space-y-4">
                    <section class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
                        ...
                        <form id="bulk-form" method="POST" action="{{ route('management.pkpa-placement-plans.bulk.preview', $plan) }}" class="mt-3 grid gap-3" data-placement-bulk-form data-options-url="{{ route('management.pkpa-placement-plans.options', $plan) }}">
                            ...
                            <select name="site_field_supervisor_id" class="rounded-xl border border-slate-300 px-3 py-2 text-sm" data-bulk-field-select><option value="">Preseptor (isi nanti)</option>@foreach($fieldSupervisors as $supervisor)<option value="{{ $supervisor->id }}">{{ $supervisor->name_snapshot }} - {{ $supervisor->practiceSite?->name }}</option>@endforeach</select>
...
(() => {
    ...
    domainSelect?.addEventListener('change', async () => {
        ...
        fillSelect(fieldSelect, 'Preseptor (isi nanti)', [], () => '');
