...
@else
    ...
    <div class="space-y-2">
        ...
        <details class="rounded-xl border border-slate-200 bg-white p-2">
            ...
            <form method="POST" action="{{ route('management.pkpa-placement-plans.assignments.store', $plan) }}" class="mt-3 grid gap-2">
                @csrf
                ...
                <select name="pkpa_program_site_id" class="w-full rounded-lg border border-slate-300 px-2 py-2 text-xs" required>
                    <option value="">Tempat Praktik</option>
                    @foreach(($siteOptions[$requirement->practice_domain_id] ?? collect()) as $site)
                        <option value="{{ $site->id }}" @selected($assignment?->pkpa_program_site_id === $site->id)>{{ $site->practiceSite?->name }}{{ $site->practiceDomainOption ? ' / '.$site->practiceDomainOption->name : '' }}</option>
                    @endforeach
                </select>
                <select name="pkpa_site_availability_period_id" class="w-full rounded-lg border border-slate-300 px-2 py-2 text-xs" required>
                    <option value="">Availability</option>
                    @foreach(($siteOptions[$requirement->practice_domain_id] ?? collect()) as $site)
                        @foreach($site->availabilityPeriods as $period)
                            <option value="{{ $period->id }}" @selected($assignment?->pkpa_site_availability_period_id === $period->id)>{{ $site->practiceSite?->name }} / {{ $period->start_date->format('d M') }} - {{ $period->end_date->format('d M Y') }} / {{ $period->plannedAvailableSlots() }} slot</option>
                        @endforeach
                    @endforeach
                </select>
                <div class="grid grid-cols-2 gap-2">
                    <input type="date" name="start_date" value="{{ $assignment?->start_date?->toDateString() }}" class="rounded-lg border border-slate-300 px-2 py-2 text-xs" required>
                    <input type="date" name="end_date" value="{{ $assignment?->end_date?->toDateString() }}" class="rounded-lg border border-slate-300 px-2 py-2 text-xs" required>
                </div>
                ...
                <select name="site_field_supervisor_id" class="w-full rounded-lg border border-slate-300 px-2 py-2 text-xs" required>
                    <option value="">Pembimbing Lapangan</option>
                    @foreach(($siteOptions[$requirement->practice_domain_id] ?? collect()) as $site)
                        @foreach(($fieldOptions[$site->practice_site_id] ?? collect()) as $supervisor)
                            <option value="{{ $supervisor->id }}" @selected($assignment?->supervisors->firstWhere('supervisor_type', 'field')?->site_field_supervisor_id === $supervisor->id)>{{ $site->practiceSite?->name }} - {{ $supervisor->name_snapshot }} / max {{ $supervisor->maximum_active_students ?? '?' }}</option>
                        @endforeach
                    @endforeach
                </select>
