...
<div class="space-y-5">
    ...
    @if(! empty($pageDescription))
        <div class="rounded-2xl border border-cyan-100 bg-cyan-50/70 px-4 py-3 text-sm text-cyan-900">{{ $pageDescription }}</div>
    @endif
    <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        <div class="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
            ...
            <a href="{{ route('management.pkpa-program-sites.create') }}" class="rounded-xl bg-cyan-700 px-4 py-2 text-center text-sm font-black text-white">{{ ($pageMode ?? 'sites') === 'preceptors' ? 'Tambah Tempat PKPA' : 'Tambah Tempat' }}</a>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                ...
                @forelse($programSites as $programSite)
                    <tr>
                        ...
                        <td class="px-4 py-4 text-right"><a href="{{ ($pageMode ?? 'sites') === 'preceptors' ? route('management.pkpa-preceptors.show', $programSite) : route('management.pkpa-program-sites.show', $programSite) }}" class="rounded-lg border border-cyan-200 px-3 py-1.5 text-xs font-bold text-cyan-700">Kelola</a></td>
                    </tr>
                @empty
