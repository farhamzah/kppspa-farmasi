...
@section('content')
@php($reportQuery = array_filter(['program' => $filters['program'] ?? null, 'domain' => $filters['domain'] ?? null, 'site' => $filters['site'] ?? null, 'q' => $filters['q'] ?? null]))
<div class="space-y-5">
    <section class="border-b border-slate-200 bg-white px-6 py-5">
        ...
        <form class="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-[1.4fr_1fr_1.4fr_1.4fr_auto]">
            ...
            <button class="rounded-md bg-cyan-700 px-4 py-2 text-sm font-bold text-white">Terapkan</button>
        </form>
    </section>
