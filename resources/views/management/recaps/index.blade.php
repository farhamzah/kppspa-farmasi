...
@section('content')
@php($reportQuery = array_filter(request()->only(['program', 'domain', 'site', 'q'])))
<div class="space-y-5">
    ...
    <section class="border-y border-slate-200 bg-white px-6 py-5">
        <form method="get" class="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.4fr_1fr_1.4fr_1.4fr_auto]">
            ...
            <div class="flex items-end gap-2"><button class="min-h-10 rounded-md bg-cyan-700 px-4 text-sm font-bold text-white hover:bg-cyan-800">Terapkan</button>@if($reportQuery)<a href="{{ route('management.recaps.index') }}" class="inline-flex min-h-10 items-center rounded-md border border-slate-300 px-3 text-sm font-bold text-slate-600">Reset</a>@endif</div>
        </form>
    </section>
        ...
        <div class="grid gap-3 lg:grid-cols-2">
            @foreach($reports as $type => $report)
                <article class="border-b border-slate-200 bg-white p-5 sm:rounded-lg sm:border">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div class="min-w-0"><h4 class="font-black text-slate-950">{{ $report['title'] }}</h4><p class="mt-1 text-sm leading-6 text-slate-600">{{ $report['description'] }}</p></div><a href="{{ route('management.recaps.'.$type, $reportQuery) }}" class="shrink-0 rounded-md bg-cyan-700 px-4 py-2 text-center text-sm font-bold text-white hover:bg-cyan-800">Buka</a></div>
