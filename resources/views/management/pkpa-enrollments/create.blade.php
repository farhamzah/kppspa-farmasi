    ...
    <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <form method="POST" action="{{ route('management.pkpa-enrollments.store') }}" class="space-y-5">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div><label class="text-xs font-black uppercase tracking-widest text-slate-500">Core User ID</label><input name="core_user_id" value="{{ old('core_user_id') }}" placeholder="CORE-001" class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"></div>
                <div><label class="text-xs font-black uppercase tracking-widest text-slate-500">NPM</label><input name="student_number" value="{{ old('student_number') }}" placeholder="231001" class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"></div>
            </div>
