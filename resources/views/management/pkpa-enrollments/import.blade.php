...
<div class="space-y-5">
    ...
    <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div><p class="text-xs font-black uppercase tracking-widest text-cyan-700">Preview wajib sebelum final</p><h2 class="mt-2 text-xl font-black">Upload CSV/XLSX peserta</h2><p class="mt-1 max-w-2xl text-sm text-slate-600">Kolom wajib: core_user_id, npm, group_code, notes. Password dan role akan ditolak.</p></div>
        ...
        <form method="POST" action="{{ route('management.pkpa-enrollments.import.preview') }}" enctype="multipart/form-data" class="mt-6 grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end">
            ...
            <button class="rounded-xl bg-cyan-700 px-4 py-2 text-sm font-black text-white">Preview</button>
        </form>
    </div>
