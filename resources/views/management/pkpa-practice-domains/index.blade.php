    ...
    <div class="grid gap-4 lg:grid-cols-2">
        @forelse($domains as $domain)
            <article class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
                ...
                <div class="mt-4 grid grid-cols-2 gap-3 text-sm"><div class="rounded-xl bg-slate-50 p-3"><p class="text-xs font-black uppercase text-slate-500">Pilihan</p><p class="font-black">{{ $domain->options_count }}</p></div><div class="rounded-xl bg-slate-50 p-3"><p class="text-xs font-black uppercase text-slate-500">Tempat</p><p class="font-black">{{ $domain->practice_sites_count }}</p></div></div>
                <div class="mt-4 flex flex-wrap gap-2"><a href="{{ route('management.pkpa-practice-domains.show', $domain) }}" class="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-bold">Detail</a><a href="{{ route('management.pkpa-practice-domains.edit', $domain) }}" class="rounded-lg border border-cyan-200 px-3 py-1.5 text-xs font-bold text-cyan-700">Edit</a></div>
            </article>
        @empty
