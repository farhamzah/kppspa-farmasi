<!doctype html>
<html lang="id">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>{{ $title }}</title>
    <style>
        @page { size: A4 landscape; margin: 10mm; }
        * { box-sizing: border-box; }
        body { color: #0f172a; font-family: Arial, sans-serif; margin: 24px; }
        h1 { font-size: 22px; margin: 0 0 4px; }
        .subtitle { color: #475569; font-size: 13px; margin-bottom: 18px; }
        .toolbar { display: flex; gap: 8px; justify-content: flex-end; margin-bottom: 16px; }
        .toolbar button, .toolbar a { background: white; border: 1px solid #cbd5e1; border-radius: 6px; color: #0f172a; padding: 8px 12px; text-decoration: none; }
        .meta { display: grid; font-size: 11px; gap: 6px; grid-template-columns: repeat(5, 1fr); margin-bottom: 16px; }
        .meta div { border: 1px solid #e2e8f0; padding: 7px; }
        table { border-collapse: collapse; font-size: 9px; width: 100%; }
        th, td { border: 1px solid #dbe3ef; padding: 7px; text-align: left; vertical-align: top; }
        th { background: #f1f5f9; color: #475569; font-size: 8px; text-transform: uppercase; }
        thead { display: table-header-group; }
        tr { break-inside: avoid; page-break-inside: avoid; }
        @media print { body { margin: 0; } .toolbar { display: none; } }
        @media (max-width: 768px) { .meta { grid-template-columns: 1fr; } }
    </style>
</head>
<body @if($printMode) onload="window.print()" @endif>
    <div class="toolbar">
        <button onclick="window.print()">Print</button>
        <button onclick="window.close()">Tutup</button>
    </div>
    <h1>{{ $title }}</h1>
    <div class="subtitle">MY PKPA Farmasi UBP</div>
    <div class="meta">
        @foreach($filters + ['Total data' => $rows->count()] as $label => $value)
            <div><strong>{{ $label }}:</strong><br>{{ $value }}</div>
        @endforeach
    </div>
    <table>
        <thead><tr>@foreach(array_keys($rows->first() ?? ['No' => '', 'Data' => '']) as $heading)<th>{{ $heading }}</th>@endforeach</tr></thead>
        <tbody>
            @forelse($rows as $row)
                <tr>@foreach($row as $value)<td>{{ $value }}</td>@endforeach</tr>
            @empty
                <tr><td colspan="20">Belum ada data sesuai filter.</td></tr>
            @endforelse
        </tbody>
    </table>
</body>
</html>
